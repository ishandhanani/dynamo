...
use crate::{
    ...
    protocols::{Annotated, http::ResponseFrame},
};
...
#[tokio::test]
async fn public_native_generate_keeps_bytes_books_http_request_and_fences_retired_workers() {
    tokio::time::timeout(Duration::from_secs(15), async {
        ...
        card.runtime_config.runtime_data.insert(
            crate::protocols::sglang::HTTP_CAPABILITY.into(),
            true.into(),
        );
        let binding = Arc::new(NativeGenerateBinding {
            ...
            client: HttpClient::from_client_with_dispatch(
                client.clone(),
                RouterMode::Direct,
                engine.clone(),
            )
        ...
        for (index, (body, rows)) in [(BODY, 1), (BEAM_BODY, 1)].into_iter().enumerate() {
