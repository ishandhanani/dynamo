...
use super::*;
use crate::http::service::native_generate::BodyProgress;
...
impl NativeGenerateBinding {
    ...
    pub(super) async fn forward_disaggregated(
        ...
        let return_logprob = projection.value.get("return_logprob").is_some_and(|value| {
        ...
        let decode = match heads {
            Heads::Ready(decode, prefill_body) => {
                if return_logprob {
                    logprobs::merge_response(decode, prefill_body).await?
                } else {
                    decode
                }
            }
            Heads::Rejected(response) => response,
        };
...
enum Heads {
    Ready(Response, tokio::sync::oneshot::Receiver<Bytes>),
    Rejected(Response),
}
    ...
    let join = async {
        ...
        let decode = tokio::select! {
            prefill = &mut prefill => {
                ...
                drain_prefill(prefill, cancellation.clone(), collect_prefill, prefill_tx);
                decode.await?
            },
            decode = &mut decode => {
                let decode = decode?;
        ...
        Ok(Heads::Ready(decode, prefill_rx))
    };
    tokio::select! {
        biased;
...
/// Read prefill concurrently so transport failure cancels a pending decode.
/// Only retain its body when the caller requested prompt logprobs.
fn drain_prefill(
    prefill: Response,
    drain_cancel: CancellationToken,
    collect: bool,
    completed: tokio::sync::oneshot::Sender<Bytes>,
) {
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                chunk = body.next() => match chunk {
                    None => { let _ = completed.send(result.freeze()); return; },
                    Some(Ok(chunk)) => {
                        if collect {
                            if result.len().saturating_add(chunk.len()) > logprobs::MAX_BODY {
                                tracing::warn!("native prefill logprob response exceeds body limit");
                                drain_cancel.cancel();
                                return;
                            }
...
mod tests {
    ...
    fn native_pd_projection_omits_warmups_and_rejects_conflicting_controls() {
        ...
        let children = projection.children(None, true).unwrap();
