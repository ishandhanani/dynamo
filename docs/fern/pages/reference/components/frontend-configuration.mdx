...
<ParamField path="--router-embedded-selection / --no-router-embedded-selection" type="boolean" default="true">
  Deprecated and ignored. The router's scheduling and active-sequence accounting always run on an
  embedded selection-service partition (the same core as the standalone selection service); the
  runtime-bound scheduler was removed. `--router-replica-sync` and overload detection apply to
  the embedded partition. `--no-router-embedded-selection` only logs a warning.
