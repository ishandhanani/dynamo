...
<ParamField path="--router-embedded-selection / --no-router-embedded-selection" type="boolean" default="false">
  **Experimental.** Run the router's scheduling and active-sequence accounting on an embedded
  selection-service partition (the same core as the standalone selection service) instead of the
  runtime-bound scheduler. The KV indexer, request transport, and worker discovery are unchanged;
  `--router-replica-sync` is not applied to the embedded partition.
