...
| `DYN_EPP_DATA_PARALLEL_SIZE` | No | Data-parallel ranks per pod; defaults to `1`. Rank `r` publishes KV events on `DYN_EPP_KV_EVENT_PORT + r * DYN_EPP_KV_EVENT_PORT_STRIDE`. |
| `DYN_EPP_KV_EVENT_PORT_STRIDE` | No | Port distance between consecutive ranks' KV event ports; defaults to `1`. |
...
| `DYN_EPP_PREFILL_INFERENCE_POOL_NAME` | No | Experimental. A second `InferencePool` holding prefill workers. When set, the EPP runs decode-first disaggregated coordination: it books a decode worker from `DYN_EPP_INFERENCE_POOL_NAME`, books a prefill worker from this pool constrained to the decode worker's KV-transfer domain, and sets `x-dynamo-routing-mode: disaggregated` plus `x-dynamo-prefill-instance-id` / `x-dynamo-prefill-dp-rank` on the forwarded request. The Envoy subset hint is not applied in this mode. |
| `DYN_EPP_DISAGG_PREFILL_FAILURE` | No | With a prefill pool: `free-decode` (default) releases the decode booking and fails the request when prefill cannot be booked; `bypass-on-decode` keeps decode and runs prefill there (`x-dynamo-routing-mode: aggregated`). |
| `DYN_EPP_DISAGG_ORDER` | No | With a prefill pool: `decode-anchored` (default) books decode first and constrains prefill to its KV-transfer domain; `prefill-anchored` books prefill first (KV-aware) and constrains decode to the prefill worker's domain, the order the Dynamo frontend uses. |
